...
const TableInfoMapSeguridad = ({departamento, municipio, titleDb, onMarkerClick, vereda}) => {

  const departamentoUpper = departamento.toUpperCase();
  const municipioUpper = municipio.toUpperCase();
  const veredaFormatted = vereda.toLowerCase().replace(/-/g, ' ');

  
  /* const filteredData = dataFactor.filter(item => item.DEPARTAMENTO === departamentoUpper && item.MUNICIPIO === municipioUpper); */
  ...
  const filteredData = dataFactor.filter(
    (item) =>
      ...
      (item.FACTOR_SIMPLIFICADO.toLowerCase() === 'seguridad' && item.NOMBRE.toLowerCase() === veredaFormatted)
  );
  ...
  return (
    <div className="content w-full">
      <div className="overflow-x-auto h-full">
        <div className="title">
          <h2 className='text-xl underline font-bold mb-10'>{municipio} | {vereda} | {titleDb}</h2>
        </div>
              ...
              <tr key={index}>
                ...
                <td>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-bold">{item.CARENCIA.charAt(0).toUpperCase() + item.CARENCIA.slice(1).toLowerCase()}</div>
                      <div className="text-sm opacity-50">{item.FACTOR_SIMPLIFICADO.charAt(0).toUpperCase() + item.FACTOR_SIMPLIFICADO.slice(1).toLowerCase()}</div>
                    </div>
                  </div>
                </td>
                <td>
                ...
                <td>
                  <div className="selectActorEject flex gap-2 items-center">
                    <input
                      className='hidden'
                      type="checkbox"
                      name={`lockedInput-${index}-${item.FACTOR_SIMPLIFICADO}`}
                      id={`lockedInput-${index}-${item.FACTOR_SIMPLIFICADO}`}
                      checked={checkedItems[index]}
                      onChange={() => handleCheckboxChange(index)}
                    />
                    <Tooltip id="tooltipLocked"/>
                    <label htmlFor={`lockedInput-${index}-${item.FACTOR_SIMPLIFICADO}`} data-tooltip-id="tooltipLocked" data-tooltip-content={` ${checkedItems[index] ? 'Bloquear' : 'Desbloquear'}`} >
                ...
                {/* <th>
                  <div className="btnAction flex gap-2">
                    <Tooltip className='font-normal' id="tooltipBtnAction"/>
                    <label htmlFor={`viewMarker-${index}-${item.FACTOR_SIMPLIFICADO}`} className="btn font-normal" data-tooltip-id="tooltipBtnAction" data-tooltip-content='Ver en mapa satelital' >
                    ...
                    <div className="btn" data-tooltip-id="tooltipBtnAction" data-tooltip-content='Descargar último informe'>
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
