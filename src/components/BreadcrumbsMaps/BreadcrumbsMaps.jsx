...
const BreadcrumbsMaps = ({departamento, municipio, vereda}) => {
  ...
  const linkMunicipio =  municipio.toLowerCase().replace(/\s+/g, '-');
  
  //console.log(linkMunicipio)
  return (
    <div className='w-full'>
      <div className="text-sm breadcrumbs p-0">
        <ul className='p-0'>
          ...
          <li className='m-0 color text-slate-400'>
            <Link to={`/departamento/${departamento}/municipio/${linkMunicipio}`}>{formatMunicipio}</Link>
          </li> 
